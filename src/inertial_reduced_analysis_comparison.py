...
OUTPUT_TABLE_FILE = 'resources/verified_inertial_reduced/latex_tables.tex'
...
\\begin{{table}}[t]
    ...
    \\end{{tabular}}
    \\vspace{{1ex}}
    \\caption{{Inertial system feasibility results for continuous problems, comparing the number of feasible instances when fixing binary values predicted by different methods.}}
    \\vspace{{-0.5cm}}
    \\label{{tab:inertial-feasibility}}
\\end{{table}}
    """
    return latex_code
...
"""
    ...
    \end{tabular}
    \vspace{1ex}
    \caption{Inertial system suboptimality results for continuous problems, comparing the methods by the number of solutions within various suboptimality percentage ranges}
    \vspace{-0.5cm}
    \label{tab:inertial-suboptimality}
\end{table}
"""
...
\\begin{{table}}[t]
    ...
    \\end{{tabular}}
    \\vspace{{1ex}}
    \\caption{{Inertial system relative decrease in solving times of problem~\\eqref{{eq:mpMIP_deltafixed}} (fixed binary variables) relative to problem~\\eqref{{eq:mpMIP}}.}}
    \\vspace{{-0.5cm}}
    \\label{{tab:inertial-times-table}}
\\end{{table}}
    """
...
if __name__ == '__main__':
    ...
    my_cond = (data['s_miqp'] == 'Solved')
    my_cond_2 = data['s_qp'] != 'solved'

    cond = data['s_miqp'] == 'Solved'
    feas = data[cond]
    cond_fd = data['s_qp'] == 'solved'
    feas_fd = data[cond_fd]
    cond_dt = data['s_qp_dt'] == 'solved'
    ncond_dt = data['s_qp_dt'] != 'solved'
    feas_dt = data[cond_dt]
    cond_rf = data['s_qp_rf'] == 'solved'
