...
if __name__ == '__main__':
    ...
    while i < POINTS:
        ...
        try:
            mip_sol = solve_mpc_miosqp(point)
        ...
        p_sol = solve_mpc_osqp(point, class_to_label(tree.predict(point), tree.output_size))

        p_sol_dt = solve_mpc_osqp(point, clf_dt.predict([point])[0])

        p_sol_rf = solve_mpc_osqp(point, clf_rf.predict([point])[0])
